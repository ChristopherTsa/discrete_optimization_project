...
# Fonction pour ajouter les contraintes GCS de sous-tours
function add_subtour_constraints!(model, subtours, x, n)
    for subtour in subtours
        # Ajout des contraintes GCS pour interdire le sous-tour
        @constraint(model, [k in subtour], sum(x[i, j] for i in subtour, j in subtour if i != j) <= sum(x[i, j] for i in subtour if i != k for j in 1:n))
        println("Ajout de la contrainte GCS pour le sous-tour: ", subtour)
    end
end
...
function resolution(n, D, R, Amin, d, f, coords, Nr, regions)
    ...
    # Contrainte de continuite de flow
    ...
    while true
        ...
        add_subtour_constraints!(model, subtours, x, n)
        iteration += 1
    end
